    ...
    try:
        if path_to_annotations.is_dir():
            ...
            for annotations_path in path_to_annotations.glob('*.csv'):
                annotations = pd.read_csv(annotations_path, parse_dates=['start_datetime', 'end_datetime'], date_format='ISO8601')
                annotations_list.append(annotations)
            total_annotations = pd.concat(annotations_list, ignore_index=True)
        else:
            total_annotations = pd.read_csv(path_to_annotations, parse_dates=['start_datetime', 'end_datetime'], date_format='ISO8601')
    except:
        print(path_to_annotations)
    return total_annotations


def run(predictions_path, ground_truth_path, iou_threshold=0.3, conf_threshold=None):
    """
    ...
    ground_truth = ground_truth.loc[~ground_truth.annotation.isna()]
    if conf_threshold is not None:
        ground_truth = ground_truth.loc[ground_truth.relative_confidence >= conf_threshold]
    ...
    print(set(predictions.annotation.unique()) - set(ground_truth.annotation.unique()))
    ...
    for (dataset_name, wav_path_name), wav_predictions in tqdm(predictions.groupby(['dataset', 'filename']),
        ...
        min_timestamp = wav_predictions.start_datetime.min() - pd.to_timedelta(2, unit='minutes')
        max_timestamp = wav_predictions.end_datetime.max() + pd.to_timedelta(2, unit='minutes')

        ground_truth_wav = ground_truth.loc[(ground_truth.start_datetime >= min_timestamp) & (ground_truth.start_datetime <= max_timestamp)]
    ...
    for dataset_name, conf_matrix_dataset in compute_confusion_matrix_per_dataset(ground_truth, predictions, all_classes):
        conf_matrix_dataset['dataset'] = dataset_name
        if final_conf_matrix is None:
            final_conf_matrix = conf_matrix_dataset
        else:
            final_conf_matrix = pd.concat([final_conf_matrix, conf_matrix_dataset], ignore_index=True)

    conf_matrix = compute_confusion_matrix(ground_truth, predictions, all_classes)
    conf_matrix['dataset'] = 'joined'
    final_conf_matrix = pd.concat([final_conf_matrix, conf_matrix], ignore_index=True)
...
if __name__ == '__main__':
    ...
    predictions_csv_path_all = pathlib.Path(input('Where are ALL the submissions in csv format?'))
    ground_truth_csv_path = pathlib.Path(input('Where are the ground truth in csv format (folder)?'))
    conf_threshold = input('conf. threshold. Leave blank to ignore')
    if conf_threshold != '':
        conf_threshold = float(conf_threshold)
    else:
        conf_threshold = None
    results = None
    for folder_submission in predictions_csv_path_all.glob('*'):
        if folder_submission.is_dir():
            print(folder_submission.name)
            results_submission = run(folder_submission, ground_truth_csv_path, conf_threshold=conf_threshold)
            results_submission['submission'] = folder_submission.name
            if results is None:
                results = results_submission
            else:
                results = pd.concat([results, results_submission])

            results['f1'] = 2 * (results['precision'] * results['recall']) / (results['precision'] + results['recall'])
            results.to_csv(predictions_csv_path_all.joinpath('global_ranking.csv'))

            results_filtered = results[['recall', 'precision', 'submission', 'dataset']].groupby(['submission', 'dataset']).mean()
            results_filtered['f1'] = 2 * (results_filtered['precision'] * results_filtered['recall']) / (results_filtered['precision'] + results_filtered['recall'])
            results_filtered.to_csv(predictions_csv_path_all.joinpath('global_ranking_filtered.csv'))